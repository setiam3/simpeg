...
class GenerateController extends Controller
{
    ...
    public function actionAdd($row=20,$iterate=10)
    {
        ...
        for($j=1;$j<=$iterate;$j++){
            ...
            \Yii::$app->db->createCommand()->batchInsert('MBiodata', ['nip', 'tmt', 'nama', 'foto', 'tempat_lahir', 'tanggal_lahir', 'agama', 'jenis_kelamin', 'nikah', 'status_pegawai', 'alamat','telepon','email','salt','password','created_date','created_by'], $datas)->execute();
        }   
         
        $time_elapsed_us = microtime(true) - $start;
        echo ($row*$iterate).' = '.$time_elapsed_us.' <br>';
    }
    public function actionAddBiodata($row=20,$iterate=10){
        $start = microtime(true);
        $faker = \Faker\Factory::create();
        $datas = [];
        for($j=1;$j<=$iterate;$j++){
            ...
            \Yii::$app->db->createCommand()->batchInsert('MBiodata', [
                'nama','tempatLahir','tanggalLahir','alamat','kabupatenKota','kecamatan','kelurahan',
                'jenisKelamin','agama','telp','email','statusPerkawinan','gelarDepan','gelarBelakang',
                'nik','foto','fotoNik','golonganDarah'
            ], $datas)->execute();
        }   
