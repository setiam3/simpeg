...
class FormulaController extends Controller
{
    ...
    public function actionUpdate($id)
    {
        $request = Yii::$app->request;
        $data=[];
        $model = MsFormula::find()
            ->where(['idpekerjaan'=>$id])->one();

        $kategory = Yii::$app->db_remun->createCommand("SELECT * FROM ms_formula f
        JOIN ms_bobot b on f.id_bobot=b.id
        WHERE f.idpekerjaan = $id")->queryAll();

        foreach ($kategory as $k){
            $data[] = ['kategory'=>$k['kategory'],'id_bobot'=>$k['id_bobot']];
        }
        ...
        if($request->isAjax){
            ...
            */
            ...
            if($request->isGet){
                ...
                foreach ($_POST['bobot'] as $a){
                    $model= MsFormula::find()->where(['id'=>$model->id]);
                    $model->load($request->post());
                    $model->id_bobot = $a;
                    $model->total_score=array_sum($score);
                    $model->estimasi=$model->total_score*(float)$pir;
